    ...
    while lst_points:
        ...
        for i in lst_points:
            ...
            if not UP and not DOWN:
                break
        list_lst = [
            [(_, dct_up[_]) for _ in dct_up.keys()],
            [(_, dct_down[_]) for _ in dct_down.keys()],
            [(_, dct_up_y[_]) for _ in dct_up_y.keys()],
            [(_, dct_down_y[_]) for _ in dct_down_y.keys()],
        ]
...
def roman(num):
    ...
    chlist = ["I"] + [
        chlist[i % len(chlist)] + "\u0304" * (i // len(chlist))
